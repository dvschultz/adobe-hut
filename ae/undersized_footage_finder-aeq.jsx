...
/*
 * Undersized Footage Finder (AEQuery version)
...
(function() {
    ...
    function collectUndersizedLayers(comp, targetWidth, targetHeight) {
        return aeq('layer', comp)
            ...
            .filter(function(layer) {
                return layer.source.width < targetWidth || layer.source.height < targetHeight;
            })
            .map(function(layer) {
                return {
    ...
    function saveTextReport(comp, items, targetWidth, targetHeight) {
        ...
        var unique = items.filter(function(item) {
            if (seen[item.filePath]) return false;
